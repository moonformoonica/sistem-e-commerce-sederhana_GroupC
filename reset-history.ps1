...
docker exec $SHIPPING php -r '$p=new PDO("sqlite:/data/shipments.sqlite");$p->exec("DELETE FROM shipments");$p->exec("DELETE FROM sqlite_sequence WHERE name=\"shipments\"");echo "shipments tersisa: ".$p->query("SELECT count(*) FROM shipments")->fetchColumn().PHP_EOL;'
